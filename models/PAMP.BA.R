...
cleanedData$PAMP.BA.Adjusted <- cleanedData$PAMP.BA.Adjusted/ cleanedData$USDARS.X.Adjusted
...
dtd.v_PAMP <- matrix(0 ,length(res), 1)
for (i in 1:length(dtd.v_PAMP)) {
  dtd.v_PAMP[i] <- (res[[i]]$par[1] - debt)/(res[[i]]$par[2]*res[[i]]$par[2])
}
