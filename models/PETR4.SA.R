...
cleanedData$PETR4.SA.Adjusted <- cleanedData$PETR4.SA.Adjusted/ cleanedData$USDARS.X.Adjusted
...
dtd.v_PETR <- matrix(0 ,length(res)-9, 1)
for (i in 1:length(dtd.v_BBA)) {
  dtd.v_PETR[i] <- (res[[i]]$par[1] - debt)/(res[[i]]$par[2]*res[[i]]$par[2])
}
