...
cleanedData$BBDC4.SA.Adjusted <- cleanedData$BBDC4.SA.Adjusted/ cleanedData$USDARS.X.Adjusted
...
dtd.v_BBDC <- matrix(0 ,length(res)-9, 1)
for (i in 1:length(dtd.v_BBDC)) {
  dtd.v_BBDC[i] <- (res[[i]]$par[1] - debt)/(res[[i]]$par[2]*res[[i]]$par[2])
}
...
cor(dtd.v_BBDC, dtd.v_ITUB)

cor_petr_vale <- matrix(0 ,length(dtd.v_VALE), 1)
for (i in length(dtd.v_VALE)) {
  cor_petr_vale[i] <- cor(dtd.v_PETR[i],dtd.v_VALE[i])
}
