...
#The firm from Argentina: YPF.BA
...
cleanedData$BBAS3.SA.Adjusted <- cleanedData$BBAS3.SA.Adjusted/ cleanedData$USDARS.X.Adjusted
...
dtd.v_BBA <- matrix(0 ,length(res)-9, 1)
for (i in 1:length(dtd.v_BBA)) {
  dtd.v_BBA[i] <- (res[[i]]$par[1] - debt)/(res[[i]]$par[2]*res[[i]]$par[2])
}

print(dtd.v_BBA)

dim(dtd.v_BBA)
cor(dtd.v_BBA, dtd.v_YBF)
