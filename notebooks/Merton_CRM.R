...
mcap <- 795000000000000  
debt <- 304000000000000
...
objective.function <- function(x, mcap, vol, debt, rho, Maturity){
  ...
  for (j in 1:length(vol)){
    e2[j] <- -vol[j]*mcap + x[2]*x[1]*pnorm(d1(x[1], debt*rho, x[2], Maturity))
    (e1*e1) + (e2[j]*e2[j])
    }}
...
res <- optim(c(seed.V, seed.sV),
              ...
              mcap=mcap, vol=vol[k], debt=debt, Maturity=Maturity,
              rho=rho)
...
dtd.v <- (res[[1]]$par - debt)/(res$par[m]*res$par[2])

#Make the dtd.v a varying object by referencing the par of a specific res.
#Find the pnorm for each dtd.v. 

#Apply the standard normal on dtd.v:
dtd.v <- pnorm(dtd.v)
